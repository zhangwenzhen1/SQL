...
select 地市,sum(日流量GB) as A类网格流量,count(*) as A类网格小区数 into 地市网格流量情况1 from [容量].[dbo].[流量系数1] where 路测网格='A'  group by 地市 order by 地市
...
(select 地市,sum(日流量GB) as B类网格流量,count(*) as B类网格小区数  from [容量].[dbo].[流量系数1] where 路测网格!='A' and 路测网格!='C'and 路测网格!='D' group by 地市) as temp 
left join 地市网格流量情况1 on 地市网格流量情况1.[地市]=temp.地市 left join
 
(select 地市,sum(日流量GB) as C类网格流量,count(*) as C类网格小区数  from [容量].[dbo].[流量系数1] where 路测网格='C'  group by 地市) as temp1
