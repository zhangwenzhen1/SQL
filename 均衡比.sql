...
select 地市,sum(日流量GB)/count(*) as 单小区平均流量GB into 业务均衡 from 容量.[dbo].[流量系数] group by 地市
...
(select 地市, count(*) as [系数小于等于0.05] from [容量].[dbo].[流量系数] where  流量系数<=0.05 and 流量系数>0 group by 地市) as temp 
left join [业务均衡] on [业务均衡].地市=temp.地市 left join
 
(select 地市, count(*) as [系数大于0.05小于等于0.2] from [容量].[dbo].[流量系数] where  流量系数>0.05 and 流量系数<=0.2 group by 地市) as temp1
 on [业务均衡].地市=temp1.地市 left join
 (select 地市, count(*) as [系数大于0.2小于等于0.5] from [容量].[dbo].[流量系数] where  流量系数>0.2 and 流量系数<=0.5 group by 地市) as temp2
 on [业务均衡].地市=temp2.地市 left join
 (select 地市, count(*) as [系数大于0.5小于等于1.5] from [容量].[dbo].[流量系数] where  流量系数>0.5 and 流量系数<=1.5 group by 地市) as temp3
 on [业务均衡].地市=temp3.地市 left join
 (select 地市, count(*) as [系数大于1.5小于等于3] from [容量].[dbo].[流量系数] where  流量系数>1.5 and 流量系数<=3 group by 地市) as temp4
 on [业务均衡].地市=temp4.地市 left join
 (select 地市, count(*) as [系数大于3小于等于5] from [容量].[dbo].[流量系数] where  流量系数>3 and 流量系数<=5 group by 地市) as temp5
 on [业务均衡].地市=temp5.地市 left join
 (select 地市, count(*) as [系数大于5] from [容量].[dbo].[流量系数] where  流量系数>5  group by 地市) as temp6
 on [业务均衡].地市=temp6.地市 left join
 (select 地市, count(*) as [性能为空] from [容量].[dbo].[流量系数] where  日流量GB =0 group by 地市) as temp0
 on [业务均衡].地市=temp0.地市 
